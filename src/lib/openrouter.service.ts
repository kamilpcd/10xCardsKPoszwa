import type { OpenRouterConfig, ModelParameters, RequestPayload, ApiResponse } from "./openrouter.types";
...
export class OpenRouterService {
  ...
  constructor(config: Partial<OpenRouterConfig>) {
    ...
    try {
      ...
      throw new OpenRouterError("Invalid configuration", "INVALID_CONFIG", undefined, error);
    }
  }
  ...
  public async sendChatMessage(message: string): Promise<ApiResponse> {
    ...
    try {
      ...
      try {
        ...
        throw new OpenRouterError("Invalid API response format", "INVALID_RESPONSE_FORMAT", undefined, error);
      }
    } catch (error) {
      ...
      throw new OpenRouterError("Failed to send chat message", "CHAT_ERROR", undefined, error);
    }
  }
}
